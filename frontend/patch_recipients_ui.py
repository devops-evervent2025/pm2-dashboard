...
# 1. Add Recipient interface + RecipientsModal component, right before NotificationBell
    ...
    <div className="fixed inset-0 bg-black/30 z-30 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-base font-semibold text-slate-800">Email notification recipients</h3>
        ...
        <div className="space-y-1.5 max-h-60 overflow-y-auto">
          {recipients.map((r) => (
            <div key={r.id} className="flex items-center justify-between px-2 py-1.5 rounded-md bg-slate-50">
...
# 2. Add showRecipients state to NotificationBell
...
# 3. Add "Manage recipients" link at the bottom of the dropdown header, and render the modal
...
new_header = '''          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-800">Notifications</p>
              <p className="text-xs text-slate-400">
                {total === 0 ? "All clear" : `${total} item${total === 1 ? "" : "s"} need attention`}
