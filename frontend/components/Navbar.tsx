...
function NotificationBell() {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-96 max-h-[28rem] overflow-y-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg z-20">
...
export default function Navbar({ crumbs }: { crumbs?: { label: string; href?: string }[] }) {
  ...
  const showNavbarLogo = isMobile || sidebarCollapsed;
          ...
          {crumbs?.map((c, i) => (
            <span key={i} className="flex items-center gap-3 text-slate-400">
              ...
              {c.href ? (
                ...
                <span className="text-slate-700 font-medium dark:text-slate-200">{c.label}</span>
              )}
            </span>
          ))}
        </div>
