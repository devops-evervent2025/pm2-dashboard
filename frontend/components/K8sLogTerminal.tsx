...
export default function K8sLogTerminal({
  clusterId,
  namespace,
  podName,
}: {
  ...
  useEffect(() => {
    ...
    const url = `${WS_URL}/ws/k8s-logs/${clusterId}/${encodeURIComponent(namespace)}/${encodeURIComponent(
      podName
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-200 bg-slate-50 rounded-t-lg">
        <span className="text-sm font-medium text-slate-700">{podName} — live logs</span>
