...
def _get_admin_emails(db: Session) -> List[str]:
    """Return every configured email-notification recipient."""
    admin_emails = {
...
def _status_banner(color, bg, label):
    return (
        '<div style="background:' + bg + ';border-left:5px solid ' + color +
        ';padding:10px 16px;margin-bottom:14px;border-radius:4px;">' +
        '<span style="color:' + color + ';font-weight:700;font-size:14px;letter-spacing:0.5px;">' +
        label + '</span></div>'
    )


def _email_wrapper(banner, server_name, detail_html, footer):
    now = datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')
    parts = []
    parts.append('<div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:auto;">')
    parts.append(banner)
    parts.append('<h2 style="margin:0 0 6px 0;font-size:18px;color:#1a1a1a;">' + server_name + '</h2>')
    parts.append('<p style="font-size:14px;color:#333;line-height:1.5;">' + detail_html + '</p>')
    parts.append('<p style="font-size:12px;color:#888;margin-top:18px;">' + footer + '<br>Checked at ' + now + '</p>')
    parts.append('</div>')
    return ''.join(parts)
...
def _send_replication_broken_email(db: Session, server_name: str, issue_type: str, state: str):
    ...
    if not admins:
        ...
        return
    if issue_type == "down":
        subject = "[PM2 Dashboard] \U0001F534 DOWN - " + server_name
        banner = _status_banner("#c0392b", "#fdecea", "\U0001F534 SERVER DOWN")
        detail = "MaxScale reports <strong>" + server_name + "</strong> as unreachable (state: <strong>" + state + "</strong>)."
    else:
        subject = "[PM2 Dashboard] \U0001F7E0 Replication broken - " + server_name
        banner = _status_banner("#d35400", "#fdf1e6", "\U0001F7E0 REPLICATION BROKEN")
        detail = ("<strong>" + server_name + "</strong> is no longer replicating - "
                  "MaxScale reports its state as <strong>" + state + "</strong> "
                  '(expected it to include "Slave").')
    body = _email_wrapper(banner, server_name, detail, "Check the MaxScale dashboard for details.")
...
def _send_replication_recovered_email(db: Session, server_name: str, issue_type: str):
    ...
    subject = "[PM2 Dashboard] \U0001F7E2 Recovered - " + server_name
    banner = _status_banner("#1e8449", "#eafaf1", "\U0001F7E2 RECOVERED")
    detail = "<strong>" + server_name + "</strong> has recovered and is now " + label + "."
    body = _email_wrapper(banner, server_name, detail, "No action needed.")
    send_email(admins, subject, body)
